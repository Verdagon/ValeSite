          ...
          <div className={ns("columns")}>
            ...
            <div className={ns("left")}>
              ...
              <div className={ns("main")}>
                ...
                <ul className={ns("content cozy")}>
                  <li className={ns()}>Close file stream.</li>
...
<span className="Prog"><span className="Struct">struct <span className="StructName">BigClass</span> <span className="Membs">&#123;<br />  <span className="Memb"><span className="MembName">a</span> <span className="Typ">A</span>;</span> {this.noteAnchor("nounique")}<br />  <span className="Memb"><span className="MembName">b</span> <span className="Typ">B</span>;</span><br />  <span className="Memb"><span className="MembName">c</span> <span className="Typ">C</span>;</span><br />  <span className="Memb"><span className="MembName">d</span> <span className="Typ">D</span>;</span><br />  <span className="Fn">fn <span className="FnName">BigClass</span><span className="Params">(<span className="Pat"><span className="Capture"><span className="CaptureName">this</span></span></span>)</span> <span className="Block">&#123;<br />    <span className="Let"><span className="Pat"><span className="Capture"><span className="CaptureName">this.a</span></span></span> = <span className="Call"><span className="CallLookup">A</span>()</span>;</span> {this.noteAnchor("nomakeunique")}<br />    <span className="Let"><span className="Pat"><span className="Capture"><span className="CaptureName">this.b</span></span></span> = <span className="Call"><span className="CallLookup">B</span>(<span className="Lend">&<span className="Lookup">a</span></span>)</span>;</span> {this.noteAnchor("noget")}<br />    <span className="Let"><span className="Pat"><span className="Capture"><span className="CaptureName">this.c</span></span></span> = <span className="Call"><span className="CallLookup">C</span>(<span className="Lend">&<span className="Lookup">a</span></span>)</span>;</span><br />    <span className="Let"><span className="Pat"><span className="Capture"><span className="CaptureName">this.d</span></span></span> = <span className="Call"><span className="CallLookup">D</span>(<span className="Lend">&<span className="Lookup">a</span></span>, <span className="Lend">&<span className="Lookup">c</span></span>)</span>;</span><span className="W"></span><br />  &#125;</span></span><br />&#125;</span></span><br /></span>
                ...
                <ul className={ns("content cozy")}>
                  <li className={ns()}>A PhoneCall points to two Accounts. The PhoneCall shouldn't outlive the Accounts; we should delete the PhoneCall before deleting a Account.</li>
                ...
                <div className={ns("content cozy")}>
                 In SQL, a foreign key constraint is a reference that cannot outlive the object (otherwise, it rejects the current change).
                </div>
                <div className={ns("content cozy")}>
                  For that reason, we call this kind of pointer a <b>constraint reference</b>.  {this.noteAnchor("950")}
                </div>
                <div className={ns("content cozy")}>
                  C++ can have constraint references. {this.noteAnchor("engines")} We just wrap {incode("shared_ptr")} in a class called {incode("owning_ptr")} which uses move semantics like {incode("unique_ptr")} and asserts {this.noteAnchor("544")} that the ref count equals 1 when it goes out of scope.
                </div>
                ...
                <div className={ns("content cozy")}>
                  The best part: constraint references can be <b>zero cost</b>! In debug mode, our {incode("owning_ptr")} and {incode("constraint_ptr")} contain their {incode("shared_ptr", "s")}, but in release mode, we can compile them to unique_ptr and raw pointer in release mode. In Vale, these are called this <b>Normal Mode</b> and <b>Fast Mode</b> respectively.
                </div>
                <div className={ns("content cozy")}>
                  Developing and testing with constraint references drastically reduces the chance of a crash or vulnerability in production. Think of it as a super-powered ASan or valgrind. It's not perfect, but it lets us alias {this.noteAnchor("alias")} with much more safety.
                </div>
                <div className={ns("content cozy")}>
                  Some applications will prefer <b>Resilient Mode</b>, where instead of halting the program when we delete an object, we call the destructor, zero out the object, and don't free it. This will keep it alive, and cause a halt when the constraint reference is <i>used</i>, rather than when it's violated.
                </div>
                <div className={ns("content cozy")}>
                  Vale's region isolation allows its references to use non-atomic ref counting, which is much faster than {incode("shared_ptr", "'s")} atomic ref-counting. Normal Mode and Resilient Mode might be slow in C++, but they're <b>very</b> fast in Vale.
                </div>
                ...
                <div className={ns("content cozy")}>
                  But, like two raccoons in a trench coat, the raw pointer can sometimes be something else: a pointer that <i>can</i> outlive what it points to, a <b>weak reference</b>.
                </div>
                <div className={ns("content cozy")}>
...
<span class="Prog"><span class="Interface">interface <span class="StructName">INetworkCallback</span> &#123;<br />  <span class="Fn">fn <span class="FnName">handleResponse</span><span class="Params">(<span class="Pat"><span class="Lend">&</span><span class="Capture"><span class="CaptureName">this</span></span></span>, <span class="Pat"><span class="Capture"><span class="CaptureName">resp</span></span> <span class="Typ">Str</span></span>)</span>;</span><br />  <span class="Fn">fn <span class="FnName">drop</span><span class="Params">(<span class="Pat"><span class="Capture"><span class="CaptureName">this</span></span></span>)</span>;</span><br />&#125;</span><br /><span class="Struct">struct <span class="StructName">Network</span> <span class="Membs">&#123;<br />  <span class="Fn">fn <span class="FnName">request</span><span class="Params">(<br />      <span class="Pat"><span class="Lend">&</span><span class="Capture"><span class="CaptureName">this</span></span></span>,<br />      <span class="Pat"><span class="Capture"><span class="CaptureName">url</span></span> <span class="Typ">Str</span></span>,<br />      <span class="Pat"><span class="Capture"><span class="CaptureName">callback</span></span> <span class="Ownership">&<span class="Typ">INetworkCallback</span></span></span>)</span><br />  <span class="Block">&#123; <span class="Lookup">...</span> &#125;</span></span><br />&#125;</span></span><br /></span>
                ...
                <div className={ns("content cozy")}>
                  Keep in mind, this weak reference works very differently from weak_ptr.
                </div>
                <div className={ns("content cozy")}>
                  When you lock a weak_ptr, you get a {incode("shared_ptr")} which will delay destruction and extend the lifetime of the object if the other {incode("shared_ptr", "s")} disappear.
                </div>
                <div className={ns("content cozy")}>
                ...
                <div className={ns("content cozy")}>
                  In our quest, single ownership accidentally solved a major recurring problem.
                </div>
                <div className={ns("content cozy")}>
                  We previously had a system where a {incode("shared_ptr", "'d")} object's destructor would remove it from the display when the last reference to it disappeared. <b>This was a terrible thing.</b> Every month, there would be a fresh bug about "I hit the delete button, but the thing is still in the view!" and it would take forever to figure out "who is keeping my object alive?" {this.noteAnchor("519")}
                </div>
                <div className={ns("content cozy")}>
                  The best part was that we knew who the owner should be. We could point right at the line where we should have let go of the last reference... but a reference was escaping, and preventing the destructor call. {this.noteAnchor("942")}
                </div>
                <div className={ns("content cozy")}>
                ...
                <div className={ns("content cozy")}>
                  Let's back up a step and talk about {incode("shared_ptr")} and {incode("weak_ptr")}. Anyone who has a {incode("shared_ptr<X>")} might be the unlucky one to call {incode("X", "s")} destructor. This is why destructors don't have parameters: every time you let go of a {incode("shared_ptr")}, you have to find some way to obtain the right arguments to pass them in, somehow. {this.noteAnchor("515")} Owning and constraint references are different: you know exactly who should be calling the destructor.
                </div>
                <div className={ns("content cozy")}>
                ...
                <ul className={ns("content")}>
                  <li className={ns()}>Exceptions: We need to call an object's destructor automatically when the exception is in flight, where do we get the parameters for that?</li>
                  <ul>
                  ...
                  <li className={ns()}>If we have a {incode("vector<MyObj>")}, how would ~vector know what to pass into ~MyObj?</li>
                  <ul>
                    <li className={ns()}>Destructors can have parameters now, so pass a "consumer" functor (or {incode("std::function<void(std::unique_ptr<MyObj>)>)")} (or {incode("fn(MyObj)Void")}), to call {incode("~MyObj")} appropriately!</li>
                  </ul>
                </ul>
                ...
                <div className={ns("content splitter")}>
                  <div className={ns("half")}>
                    ...
                    <div className={ns("content cozy")}>
                      Notice how we have to call {incode("setRollMode")} before the destructor.
                    </div>
                    <div className={ns("content")}>
                      We'd forget that all the time!
                    </div>
                    <div className={ns("content cozy")}>
                      However, now that we have destructor parameters, we can get rid of {incode("setRollMode")}, get rid of {incode("rollback_mode_")}, and use this destructor instead:
                    </div>
                    <div className={ns("content cozy")}>
<pre className="cppSnippet root"><div className="cppSnippet header">C++</div><code className="cpp">
{`virtual ~Transaction(
...
transaction->setRollMode(TUMBLE);
// invoke destructor
transaction = nullptr;
`}
</code></pre>
...
`}
                ...
                <div className={ns("content cozy")}>
                  We might want to return a destroyed object to a free-list.
                </div>
                <div className={ns("content cozy")}>
                ...
                <div className={ns("content cozy")}>
                  This is impossible in C++'s syntax (the {incode("~")} forces us to destroy {incode("this")}), {this.noteAnchor("1000")} so we'll use Vale syntax:
                </div>
...
{/*struct Transaction {
  ...
  fn commit(this, list &TransactionList) CommitStatus {
    ...;
...
<span className="Prog"><span className="Struct">struct <span className="StructName">Transaction</span> <span className="Membs">&#123;<br />  <span className="Fn">fn <span className="FnName">read</span><span className="Params">(<span className="Pat"><span className="Lend">&</span><span className="Capture"><span className="CaptureName">this</span></span></span>, <span className="Pat"><span className="Capture"><span className="CaptureName">query</span></span> <span className="Typ">ReadQuery</span></span>)</span> <span className="Typ">ReadResult</span> <span className="Block">&#123; <span className="Lookup">...</span> &#125;</span></span><br /><br />  <span className="Fn">fn <span className="FnName">commit</span><span className="Params">(<span className="Pat"><span className="Capture"><span className="CaptureName">this</span></span></span>, <span className="Pat"><span className="Capture"><span className="CaptureName">list</span></span> <span className="Ownership">&<span className="Typ">TransactionList</span></span></span>)</span> <span className="Typ">CommitStatus</span> <span className="Block">&#123;<br />    <span className="Lookup">...</span>;<br />    <span className="Call"><span className="Lookup">list</span>.<span className="CallLookup">reclaim</span>(<span className="Lookup">this</span>)</span>;<span className="W"></span> <span className="Comment">// move this into a different function</span><br />  &#125;</span></span><br /><br />  <span className="Fn">fn <span className="FnName">rollback</span><span className="Params">(<span className="Pat"><span className="Capture"><span className="CaptureName">this</span></span></span>, <span className="Pat"><span className="Capture"><span className="CaptureName">list</span></span> <span className="Ownership">&<span className="Typ">TransactionList</span></span></span>, <span className="Pat"><span className="Capture"><span className="CaptureName">mode</span></span> <span className="Typ">RollMode</span></span>)</span> <span className="Typ">RollbackStatus</span> <span className="Block">&#123;<br />    <span className="Lookup">...</span>;<br />    <span className="Call"><span className="Lookup">list</span>.<span className="CallLookup">reclaim</span>(<span className="Lookup">this</span>)</span>; <span className="Comment">// move this into a different function</span><br />    <span className="Ret">ret <span className="Lookup">SUCCESS</span>;</span><br />  &#125;</span></span><br />&#125;</span></span><br /></span>
                ...
                <div className={ns("content cozy")}>
                  By now you've noticed that Destructors can have overloads, take parameters, and return values... there's hardly anything that separates them from regular functions.
                </div>
                <div className={ns("content cozy")}>
                  In fact, in Vale, the whole "destructor" side of the language is implemented with one small feature:
                </div>
                ...
                <div className={ns("content cozy")}>
                  In one fell swoop, by removing our dependence on {incode("shared_ptr")}, we had taken one of the most gnarly corners of C++ and completely simplified it away.
                </div>
                ...
                <div className={ns("content cozy")}>
                  For example: if you have a list of Airports and a list of Planes, a Plane can't have a reference to its source and destination airports; if we didm we wouldn't ever be able to modify or remove any airports (or add them, because Vec's resizing could move the airports). {this.noteAnchor("546b")}
                </div>
                <div className={ns("content cozy")}>
                ...
                <ul className={ns("content cozy")}>
                  <li className={ns()}>unsafe keyword: discards our safety guarantees. {this.noteAnchor("409")}</li>
                  <li className={ns()}>Arc/Rc:</li>
                  <ul className={ns("content cozy")}>
                  ...
                  <ul className={ns("content cozy")}>
                    ...
                    <li className={ns()}>The Vec's usual wasted space, which is 1-2x the <i>highest</i> number {this.noteAnchor("206")} of (i64, T) elements we've had in the past.</li>
                    <li className={ns()}>Prevents RAII, because it doesn't destroy the object immediately when the "owning" index disappears. For example, a binary tree here would be a {incode("Vec<(i64, Node)>")}, where each node is owned by the Vec, not by the parent node. Who knows when the Vec would disappear and call {incode("drop")} on the elements. {this.noteAnchor("406")}</li>
                    <li className={ns()}>Requires us to pass the Vec as a parameter to wherever we want to access the object (even if we're only reading).</li>
                  </ul>
                </ul>
                ...
                <div className={ns("content")}>
                  And, unless we use {incode("unsafe")}, <b>we can't use RAII with these workarounds.</b>
                </div>
                ...
                <ul className={ns("content")}>
                  <li className={ns()}>Constraint refs so in debug, destruction can tell us who still needs the object to exist!</li>
                ...
                <div className={ns("content cozy")}>                
                  Until then, we want to hear from you! Come share your thoughts in the <a href="http://reddit.com/r/vale">r/Vale</a> subreddit or the <a href="https://discord.gg/SNB8yGH">Vale discord</a>!
                </div>
            ...
            <div className={ns("margin")}>
              ...
              <Note colorsAndPositions={this.state.noteColorsAndPositions} update={this.updateNoteSize} name="519">
                ...
                <div style={{marginTop: "8px"}}>
                  We call these "memory leaks". Yes, garbage collected languages can have memory leaks!
                </div>
                <div style={{marginTop: "8px"}}>
              ...
              <Note colorsAndPositions={this.state.noteColorsAndPositions} update={this.updateNoteSize} name="sovsraii">
                ...
                <div style={{marginTop: "8px"}}>
                  Single ownership is where every object is owned by a single reference.
                </div>
                <div style={{marginTop: "8px"}}>
              ...
              <Note colorsAndPositions={this.state.noteColorsAndPositions} update={this.updateNoteSize} name="777">
                In Vale, if you use the % operator to propagate errors upwards, which automatically calls {incode(".drop()")} on any local in scope.
                <div style={{marginTop: "8px"}}>However, if you have a local {incode("x")} which doesn't have a zero-arg {incode(".drop()")}, then you would normally hold onto the error, call the correct destructor for {incode("x")}, and then proceed to return the error upwards.
                </div>
              </Note>
              ...
              <Note colorsAndPositions={this.state.noteColorsAndPositions} update={this.updateNoteSize} name="546b">
                Rust also won't let us do the subcomponents pattern mentioned above, because there are references between the components.
              </Note>
              ...
              <Note colorsAndPositions={this.state.noteColorsAndPositions} update={this.updateNoteSize} name="1217">
                This may seem large, but using a smaller integer won't actually save us anything, because of padding in the (i64, T)s inside the Vec.
              </Note>
              ...
              <Note colorsAndPositions={this.state.noteColorsAndPositions} update={this.updateNoteSize} name="alias">
                To "alias" a pointer means to make another pointer, pointing to the same thing. Memory safety gets very tricky when aliasing gets involved, and this is one way to make it safer.
              </Note>
              ...
              <Note colorsAndPositions={this.state.noteColorsAndPositions} update={this.updateNoteSize} name="1041">
                In Vale, the memory usage is less (8b) but it applies to all objects. In Rust, we pay the cost per object and reference that uses Rc or IDs. In total, this could be more or less depending on the use case.
              </Note>
              ...
              <Note colorsAndPositions={this.state.noteColorsAndPositions} update={this.updateNoteSize} name="whatever">
                In C++, the "whatever we want" means calling the destructor, which is a bit limited because it takes no parameters and returns no useful information. We'll show how we can use RAII to make sure we call any of multiple methods which have no such restrictions!
              </Note>
